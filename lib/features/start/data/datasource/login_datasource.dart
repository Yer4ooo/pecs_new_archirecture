...
class LoginApiService {
  ...
  Future<dynamic> login(LoginRequestModel? user) =>
      _networkClient.postData<LoginResponseModel>(
        endpoint: 'login',
        body: {'identifier': user?.identifier, 'password': user?.password},
        parser: (response) {
          GetIt.I<KeyValueStorageService>().setUserData(
              loginResponseModelToJson(LoginResponseModel.fromJson(response)));
