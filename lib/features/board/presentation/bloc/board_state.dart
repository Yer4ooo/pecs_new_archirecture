...
class BoardState with _$BoardState {
  ...
  const factory BoardState.updateTabLoading() = UpdateTabLoading;

  const factory BoardState.updateTabSuccess(TabUpdateResponseModel tab) =
      UpdateTabLoaded;

  const factory BoardState.updateTabError(String message) = UpdateTabError;

  const factory BoardState.deleteTabLoading() = DeleteTabLoading;

  const factory BoardState.deleteTabSuccess(TabDeleteResponseModel tab) =
  DeleteTabLoaded;

  const factory BoardState.deleteTabError(String message) = DeleteTabError;
}
